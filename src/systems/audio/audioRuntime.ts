...
import { resolveWorldWeights, type ChapterWeights } from './audioFoundation'
export interface AudioPlayerLike { src: string; loop: boolean; preload: string; volume: number; paused: boolean; play(): Promise<void>; pause(): void; removeAttribute(name: string): void; load(): void }
...
export class ExperienceAudioRuntime {
  ...
  update(weights: ChapterWeights, deltaSeconds = 1 / 60) {
