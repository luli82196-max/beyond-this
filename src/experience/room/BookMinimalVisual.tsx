...
export default function BookMinimalVisual({ visual, attention, paperMap }: { visual: BookMinimalVisualOutput; attention: number; paperMap: THREE.Texture }) {
  ...
  return <group position={placement.position} rotation={placement.rotation} scale={placement.scale * (isOpen ? 1.14 : 1)}>
    <mesh position={[0, -layout.coverThickness * .3, 0]}><boxGeometry args={[layout.spreadWidth, layout.coverThickness, layout.spreadDepth]} /><meshStandardMaterial color="#4f392b" roughness={.98} /></mesh>
    ...
    <pointLight position={[0, .72, .22]} intensity={attention * roomLight.fillLightFactor} distance={1.8} color="#d9ad70" />
  </group>
}
